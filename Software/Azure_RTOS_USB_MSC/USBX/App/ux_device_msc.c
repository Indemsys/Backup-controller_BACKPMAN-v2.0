...
UINT STORAGE_Read(VOID *storage, ULONG lun, UCHAR *data_pointer,ULONG number_blocks, ULONG lba, ULONG *media_status)
{
  ...
  if (BSP_SD_IsDetected(SD_INSTANCE) != SD_NOT_PRESENT)
  {
    ...
    if (tx_event_flags_get(&EventFlag, SD_READ_FLAG, TX_OR_CLEAR,&ReadFlags, 100) != TX_SUCCESS)
    {
      return UX_ERROR;
    }
  }
...
UINT STORAGE_Write(VOID *storage, ULONG lun, UCHAR *data_pointer,ULONG number_blocks, ULONG lba, ULONG *media_status)
{
  ...
  if (BSP_SD_IsDetected(SD_INSTANCE) != SD_NOT_PRESENT)
  {
    ...
    if (tx_event_flags_get(&EventFlag, SD_WRITE_FLAG, TX_OR_CLEAR,&WriteFlags, 100) != TX_SUCCESS)
    {
      return UX_ERROR;
    }
  }
